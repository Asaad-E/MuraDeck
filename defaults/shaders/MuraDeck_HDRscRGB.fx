...
// scRGB is linear and unbounded, but RCAS's limiter is built around a 1.0 signal ceiling
// (AMD's peakC = (1.0, -4.0)). Above that the maths degenerates: with the old CAS this is
// exactly why sharpening silently switched itself off on anything brighter than SDR white.
// Normalise into [0,1) first and invert afterwards - what AMD's FsrRcasInputF hook is for.
float3 RcasEncode(float3 c) { c = max(c, 0.0); return c / (1.0 + c); }
float3 RcasDecode(float3 c) { c = clamp(c, 0.0, 0.999995); return c / (1.0 - c); }
...
float3 ApplyRCAS(float2 texcoord)
{
    ...
    float3 e = RcasEncode(eRaw);
    float3 b = RcasEncode(SampleOffset(texcoord, int2( 0, -1)));
    float3 d = RcasEncode(SampleOffset(texcoord, int2(-1,  0)));
    float3 f = RcasEncode(SampleOffset(texcoord, int2( 1,  0)));
    float3 h = RcasEncode(SampleOffset(texcoord, int2( 0,  1)));
    ...
    return RcasDecode(saturate(outColor));
}
